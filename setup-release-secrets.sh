...
# paste that same value instead of minting a new one.
#
# Usage:
...
echo "    Paste your calstfrancis/flatpak-scoped token (reuse one from another app's"
...
echo "  gh workflow run release-flatpak.yml --repo $REPO -f version=\$(grep '^version' pereplyot-ui-gtk/Cargo.toml | head -1 | sed 's/version = \"\\(.*\\)\"/\\1/')"
